...
if [ -f "$VCONSOLE_CONF" ]; then
  ...
  if [[ -L "$KEYBOARD_CONF_FILE" ]]; then
    rm -f "$KEYBOARD_CONF_FILE"
  fi
...
if ! command -v lsusb >/dev/null 2>&1; then
  log_info "lsusb not available. Skipping Apple keyboard detection."
elif lsusb | grep -iq "Apple, Inc. Keyboard"; then
