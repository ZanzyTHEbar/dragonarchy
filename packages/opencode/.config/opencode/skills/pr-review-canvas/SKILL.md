...
description: Generate an interactive PR review walkthrough as an HTML page. Use when the user pastes a GitHub PR URL and asks for a review, walkthrough, summary, annotated diff, or PR review canvas.
---
...
Run these `gh api` calls in parallel when the PR owner, repo, and number are known:
...
- **Pseudocode summaries** for verbose code: show the algorithm in plain English or short pseudocode, with the real diff collapsed below using a `.bp-section` card labeled "Show full implementation". Great when 150 lines of retry/backoff/error-handling code is really just "fetch with exponential backoff and circuit breaker".
- Diagrams: inline SVG, mermaid via CDN, or ASCII art in `<pre>`.
...
| `.file-card`, `.file-hdr`, `.file-body` | Collapsible file card, using `onclick="toggle(this)"` on `.file-hdr` |
| `.file-note` | Sticky reviewer annotation inside a file card |
| `.bp-section`, `.bp-hdr`, `.bp-body` | Collapsed boilerplate card, using `onclick="toggleBP(this)"` |
...
| `renderDiff(target, diffInput)` | Render a unified diff. `target` can be a DOM element, string ID, or CSS selector. `diffInput` can be a raw patch string or an array of lines. It automatically filters imports, collapses whitespace-only changes, and detects moved code with blue/purple tint. |
...
1. During the fetch step, save patches to a JSON file using `jq`:
...
1. Write your body HTML, everything inside `<body>`, to `/tmp/pr-review-{number}-body.html`.
2. Save patches to `/tmp/pr-patches-{number}.json` using the `jq` command above.
3. Run the Python assembly script from step 3 with the working directory set to this skill directory, typically `~/.config/opencode/skills/pr-review-canvas`, so it can read `styles.css`, `renderer.js`, and `template.html`.
4. Write the final HTML to `/tmp/pr-review-{number}.html`.
5. If the user wants to view it locally, start a local server on a fixed port:

   ```bash
   cd /tmp && python3 -m http.server 8432 --bind 127.0.0.1
   ```

   Run this backgrounded, then open `http://127.0.0.1:8432/pr-review-{number}.html` in a browser if a browser tool is available, or return the file path and URL to the user.

   **Why a fixed port and `cd /tmp`:** background shells have no TTY, so Python buffers its startup message. Using port 0 means you may never read which port was chosen. `--directory /tmp` works but `cd /tmp` is more robust across Python versions. If port 8432 is taken, try 8433, 8434, etc.
...
- Detects moved code blocks, three or more consecutive lines deleted in one place and added identically elsewhere, and renders them in blue/purple instead of red/green.
- Near-matches, moved plus small edit, get a different purple tint.
...
- Core files expanded by default using `.file-body.open`; mechanical files collapsed.
