---
name: loop-on-ci
description: Monitor PR checks and fix failures until green. Use when the user asks to watch a branch or pull request and iterate on CI failures using gh pr checks as the source of truth.
---
...
5. After each approved push, re-check the full PR check set and repeat until green.
...
- Do not bypass hooks with `--no-verify` to force progress.
...
- Push only when the user explicitly requested CI repair or approved pushing.
