...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `$XDG_CONFIG_HOME/tldraw/server.json` or `$HOME/.config/tldraw/server.json`.
...
A ready-made helper ships with this skill at `"${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/tldraw-offline/tq"`. Invoke it as `sh "${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/tldraw-offline/tq" <METHOD> <path> [body]` — it re-reads the port and token from `server.json` itself on every call, so you never handle the token or the fresh-shell env problem. A body starting with `{` is sent as JSON; anything else as raw `text/plain`:

```bash
sh "${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/tldraw-offline/tq" POST /api/search '{"code":"return await api.getDocs()"}'
sh "${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/tldraw-offline/tq" POST /api/doc/DOC_ID/exec 'return editor.getCurrentPageShapes().length'
sh "${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/tldraw-offline/tq" GET  /api/doc/DOC_ID/script-status
```

If `tq` is missing (an older install), fall back to raw `curl` with the `PORT`/`TOKEN` reads shown above. The raw-`curl` examples below stay in explicit form so each request is visible; translate any to `sh "${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/tldraw-offline/tq" <METHOD> <path> [body]`.
...
Shown as raw `curl`; `sh "${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/tldraw-offline/tq" <METHOD> <path> [body]` is the shorter equivalent that handles the port and token for you.
