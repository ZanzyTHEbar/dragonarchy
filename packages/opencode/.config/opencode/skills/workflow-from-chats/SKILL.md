...
description: Extract durable working preferences from recent OpenCode or Cursor chats and convert them into skills, rules, or workflow docs. Use when asked to learn preferences, mine feedback, personalize workflows, or generate team/person-specific agent guidance.
---
...
If transcript access is unavailable, use only the current conversation and explicit user-provided excerpts. Do not invent a broader evidence corpus.

## Scope

- Default to the last 7 days unless the user asks for a different window.
- Read parent transcripts and relevant subagent transcripts when they are available. Use subagent content as evidence, but cite only parent conversations.
...
2. Build an internal transcript inventory: title/topic, parent conversation ID if safe to disclose, approximate date, completion state, relevant subagents, and why it may contain preference evidence.
...
- Target workflow.
- Evidence corpus with safe parent conversation citations only.
- Preference profile.
