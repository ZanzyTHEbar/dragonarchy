...
if [[ -f "$AMDGPU_CONF" ]]; then
    ...
    if grep -qw "amdgpu.modeset=1" /proc/cmdline; then
        ...
        LIMINE_DROPIN_SRC="${DOTFILES_DIR}/hosts/firedragon/etc/limine-entry-tool.d/10-amdgpu.conf"
        LIMINE_DROPIN_DEST="/etc/limine-entry-tool.d/10-amdgpu.conf"
        if [[ -f "$LIMINE_DROPIN_SRC" ]]; then
            sudo mkdir -p /etc/limine-entry-tool.d
            sudo cp -f "$LIMINE_DROPIN_SRC" "$LIMINE_DROPIN_DEST"
            log_success "Installed Limine kernel parameter drop-in for amdgpu.modeset=1"
        else
        ...
        if command -v limine-update >/dev/null 2>&1; then
            if sudo limine-update; then
                ...
                log_warning "limine-update failed; please regenerate Limine entries manually"
            fi
            ...
            if sudo limine-mkinitcpio; then
                ...
                log_warning "limine-mkinitcpio failed; please regenerate Limine entries manually"
            fi
        else
